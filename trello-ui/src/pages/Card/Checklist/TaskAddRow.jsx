...
export default function TaskAddRow({ boardMembers, onAdd, onCancel, disabled = false }) {
    ...
    return (
        <div className="add-task-section">
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="input-field" placeholder="Add an item" style={{ marginBottom: "1rem" }} disabled={disabled} />

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: ".5rem", position: "relative" }}>
                    <Button icon="material-symbols:person" variant="blueModern" iconSize={24} size="md" onClick={() => setShowAssign((s) => !s)} disabled={disabled} />

                    <AssignDropdown open={showAssign} members={boardMembers} selectedIds={assignedIds} onToggle={toggleAssign} anchor="right" />

                    <div style={{ position: "relative" }}>
                        <Button icon="material-symbols:schedule" variant="greenModern" iconSize={24} size="md" onClick={() => setShowDue((s) => !s)} disabled={disabled} />
                ...
                <div style={{ display: "flex", gap: ".5rem" }}>
                    <Button
                        ...
                        iconSize={24}
                        size="md"
                        onClick={() => onAdd({ title, assignedIds, dueDate })}
